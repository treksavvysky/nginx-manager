...
class TestCertRequestSuggestions:
    ...
    def test_suggestions_not_reloaded(self):
        ...
        suggestions = get_cert_request_suggestions(domain="example.com", reloaded=False, sites_using_cert=[])
    ...
    def test_suggestions_no_sites(self):
        ...
        suggestions = get_cert_request_suggestions(domain="example.com", reloaded=True, sites_using_cert=[])
    ...
    def test_suggestions_include_test(self):
        ...
        suggestions = get_cert_request_suggestions(
            domain="example.com", reloaded=True, sites_using_cert=["example.com"]
        )
...
class TestCertDiagnosticSuggestions:
    ...
    def test_suggestions_dns_not_resolving(self):
        ...
        suggestions = get_cert_diagnostic_suggestions(
            domain="example.com", dns_ok=False, port_80_ok=False, has_cert=False
        )
    ...
    def test_suggestions_port_80_not_open(self):
        ...
        suggestions = get_cert_diagnostic_suggestions(
            domain="example.com", dns_ok=True, port_80_ok=False, has_cert=False
        )
    ...
    def test_suggestions_ready_no_cert(self):
        ...
        suggestions = get_cert_diagnostic_suggestions(
            domain="example.com", dns_ok=True, port_80_ok=True, has_cert=False
        )
        cert_suggestions = [
            s for s in suggestions if "certificate" in s["action"].lower() or "ssl" in s["action"].lower()
        ]
        assert len(cert_suggestions) >= 1

    def test_suggestions_has_cert(self):
        """Test suggestions when certificate exists."""
        suggestions = get_cert_diagnostic_suggestions(domain="example.com", dns_ok=True, port_80_ok=True, has_cert=True)
