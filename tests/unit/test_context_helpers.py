...
class TestSiteCreateSuggestions:
    ...
    def test_suggestions_when_not_reloaded(self):
        ...
        suggestions = get_site_create_suggestions(
            site_name="test.local", site_type="static", reloaded=False, enabled=True
        )

        # Should have reload suggestion as high priority
        reload_suggestion = next((s for s in suggestions if "reload" in s["action"].lower()), None)
    ...
    def test_no_reload_suggestion_when_reloaded(self):
        ...
        suggestions = get_site_create_suggestions(
            site_name="test.local", site_type="static", reloaded=True, enabled=True
        )

        # Should not have reload suggestion
        reload_suggestion = next((s for s in suggestions if "reload nginx" in s["action"].lower()), None)
    ...
    def test_static_site_suggestions(self):
        ...
        root_suggestion = next((s for s in suggestions if "root" in s["action"].lower()), None)
    ...
    def test_reverse_proxy_suggestions(self):
        ...
        suggestions = get_site_create_suggestions(
            site_name="api.local", site_type="reverse_proxy", reloaded=False, enabled=True
        )

        # Should have upstream verification suggestion
        upstream_suggestion = next((s for s in suggestions if "upstream" in s["action"].lower()), None)
...
class TestSiteDeleteSuggestions:
    ...
    def test_reload_suggestion_when_was_enabled(self):
        ...
        suggestions = get_site_delete_suggestions(site_name="test.local", reloaded=False, was_enabled=True)
    ...
    def test_no_reload_for_disabled_site(self):
        ...
        suggestions = get_site_delete_suggestions(site_name="test.local", reloaded=False, was_enabled=False)

        # Should not have high priority reload suggestion
        reload_suggestion = next(
            (s for s in suggestions if "reload nginx" in s["action"].lower() and s["priority"] == "high"), None
        )
        assert reload_suggestion is None
...
class TestConfigWarnings:
    ...
    def test_localhost_proxy_warning(self):
        ...
        warnings = get_config_warnings(proxy_pass="http://localhost:3000")

        localhost_warning = next((w for w in warnings if w["code"] == "localhost_proxy"), None)
    ...
    def test_port_443_without_ssl_warning(self):
        ...
        warnings = get_config_warnings(ssl_enabled=False, listen_ports=[443])

        ssl_warning = next((w for w in warnings if w["code"] == "port_443_no_ssl"), None)
        assert ssl_warning is not None

    def test_ssl_without_cert_warning(self):
        """Test warning for SSL enabled without certificate."""
        warnings = get_config_warnings(ssl_enabled=True, has_ssl_cert=False)

        cert_warning = next((w for w in warnings if w["code"] == "ssl_no_cert"), None)
        assert cert_warning is not None

    def test_no_warnings_for_valid_config(self):
        """Test no warnings for a properly configured site."""
        warnings = get_config_warnings(ssl_enabled=False, listen_ports=[80], root_path="/var/www/html")
...
class TestNginxReloadSuggestions:
    ...
    def test_success_suggestions(self):
        ...
        suggestions = get_nginx_reload_suggestions(success=True, health_verified=True, auto_rolled_back=False)
    ...
    def test_rolled_back_suggestions(self):
        ...
        suggestions = get_nginx_reload_suggestions(success=False, health_verified=False, auto_rolled_back=True)
...
class TestNginxRestartSuggestions:
    ...
    def test_success_suggestions(self):
        ...
        suggestions = get_nginx_restart_suggestions(success=True, health_verified=True)
    ...
    def test_failed_health_check_suggestions(self):
        ...
        suggestions = get_nginx_restart_suggestions(success=True, health_verified=False)
...
class TestSystemStateSummary:
    ...
    def test_healthy_system(self):
        ...
        summary = get_system_state_summary(
            nginx_running=True, nginx_healthy=True, total_sites=5, enabled_sites=4, disabled_sites=1
        )
    ...
    def test_degraded_system(self):
        ...
        summary = get_system_state_summary(
            nginx_running=True, nginx_healthy=False, total_sites=2, enabled_sites=2, disabled_sites=0
        )
    ...
    def test_down_system(self):
        ...
        summary = get_system_state_summary(
            nginx_running=False, nginx_healthy=False, total_sites=2, enabled_sites=2, disabled_sites=0
        )
...
class TestErrorContext:
    ...
    def test_docker_unavailable_context(self):
        ...
        context = get_error_context(
            error_type="docker_unavailable", error_message="Cannot connect to Docker daemon", operation="reload"
        )
...
class TestSuggestionAndWarningModels:
    ...
    def test_suggestion_to_dict(self):
        ...
        suggestion = Suggestion(
            action="Test action", reason="Test reason", endpoint="/test", priority=SuggestionPriority.HIGH
        )
    ...
    def test_suggestion_without_endpoint(self):
        ...
        suggestion = Suggestion(action="Test action", reason="Test reason")
    ...
    def test_warning_to_dict(self):
        ...
        warning = Warning(code="test_warning", message="Test message", suggestion="Fix it")
