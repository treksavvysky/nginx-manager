...
async def step_check_prerequisites(context: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not status.get("running"):
            return {"success": False, "message": "NGINX container is not running. Start it before creating a site."}
    except DockerServiceError as e:
        return {"success": False, "message": f"Cannot connect to NGINX container: {e.message}"}
    ...
    if disabled_file.exists():
        return {"success": False, "message": f"Site '{name}' exists but is disabled. Enable or delete it first."}

    return {"success": True, "message": "Prerequisites satisfied: NGINX running, site name available"}
...
async def step_verify_site(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if result.get("success"):
        return {"success": True, "message": f"Site '{context['name']}' is configured and NGINX config is valid"}
    return {
        "success": False,
...
async def step_diagnose_ssl(context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"success": True, "message": f"SSL prerequisites verified for {domain}", "data": result}
...
async def step_request_certificate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    result = await request_certificate(
        domain=domain, alt_names=alt_names, auto_renew=context.get("auto_renew", True), auto_reload=True, dry_run=False
    )
    return result
...
async def step_verify_ssl(context: dict[str, Any]) -> dict[str, Any]:
    ...
    result = await nginx_test()
    if result.get("success"):
        return {"success": True, "message": "SSL certificate installed and NGINX config valid"}
    return {
        "success": False,
...
async def step_verify_site_exists(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if not conf_file.exists():
        return {"success": False, "message": f"Site '{name}' not found. Cannot migrate a non-existent site."}

    return {"success": True, "message": f"Site '{name}' found and ready for migration"}
...
async def step_test_config_after_update(context: dict[str, Any]) -> dict[str, Any]:
    ...
    result = await nginx_test()
    if result.get("success"):
        return {"success": True, "message": "NGINX configuration valid after migration"}
    return {
        "success": False,
