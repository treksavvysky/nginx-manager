...
class NginxConfigParser:
    ...
    def parse_config_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return None    
    def _extract_server_name(self, content: str) -> Optional[str]:
    ...
    def _extract_root(self, content: str) -> Optional[str]:
        """Extract root directive."""
        match = self.root_pattern.search(content)
        return match.group(1).strip() if match else None
    
    def _extract_proxy_pass(self, content: str) -> Optional[str]:
